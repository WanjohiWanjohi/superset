...
```
celery --app=superset.tasks.celery_app:app beat ^*^ celery version < 5
```
```
celery -A superset.tasks.celery_app beat ^*^ celery version >= 5
```
...
```
celery --app=superset.tasks.celery_app:app flower ^*^ celery version < 5
```
```
celery -A superset.tasks.celery_app flower ^*^ celery version >= 5
```
